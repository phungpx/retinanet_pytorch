...
def load_backbone(backbone: str = 'resnet18', pretrained: bool = False, num_layers: Optional[int] = None):
    ...
    if backbone in resnet_layers_channels:
        backbone = ResNet(backbone, pretrained=pretrained)
        layers_channels = resnet_layers_channels[backbone]
    elif backbone in densenet_layers_channels:
        backbone = DenseNet(backbone, pretrained=pretrained)
        layers_channels = densenet_layers_channels[backbone]
    else:
